...
def txttohtml(the_text):
    ...
    if T == 1 and len(hero_list):
        ...
        for hero in hero_list:
            ...
            if hero == the_text:
                return
            he_key_word = [hero_key_word.get(t) for t in emptylist]
            key = 0
            for he in he_key_word:
                if he:
                    key = 1;
...
try:
    ...
    items_object.close()
log_text_e, abilitys_text_e, heros_text_e, items_text_e = [line.strip() for line in log_text], \
                                                          [line.strip() for line in abilitys_text],\
                                                          [line.strip() for line in heros_text],\
                                                          []
final_log_text = []
...
for i, log in enumerate(final_log_text):
    ...
    if log[1] != nowlog1 and i != (final_log_text_length - 1):  # 不相同，不是最后一个 正常处理
        ...
        if d['ability']:
            s = "{{{{Changelogdefineability{ability}{cooldown}{manacost}{septer}{othereffect}\r}}}}\r".format(
                ability=d['ability'], cooldown=d['cooldown'], manacost=d['mana cost'], septer=d['septer'],
                othereffect=d['other effect'])
            # print(s)
        ...
        if d['hero']:
            s = "{{{{Changelogdefinehero\r|newhero='否'{hero}{strength}{intelligence}{agility}{strengthgrowth}{intelligencegrowth}{agilitygrowth}{attackdamage}{healthregen}{manaregen}{armor}{magicresistance}{movementspeed}{attackrange}{attackpoint}{attackbackswing}{baseattacktime}{missilespeed}{sightrangeday}{sightrangenight}{turnrate}{collisionsize}{legs}{other}{talent}\r}}}}\r".format(
                hero=d['hero'], strength=d['strength'], intelligence=d['intelligence'], agility=d['agility'],
                strengthgrowth=d['strength growth'], intelligencegrowth=d['intelligence growth'],
                agilitygrowth=d['agility growth'], attackdamage=d['attack damage'], healthregen=d['health regen'],
                manaregen=d['mana regen'], armor=d['armor'], magicresistance=d['magic resistance'],
                movementspeed=d['movement speed'], attackrange=d['attack range'], attackpoint=d['attack point'],
                attackbackswing=d['attack backswing'], baseattacktime=d['base attack time'],
                missilespeed=d['missile speed'], sightrangeday=d['sight range day'],
                sightrangenight=d['sight range night'], turnrate=d['turn rate'], collisionsize=d['collision size'],
                legs=d['legs'], other=d['other'], talent=d['talent'])
            transto_text.write(s)
        nowlog1 = log[1]
        nowlog0 = log[0]
        d = {
